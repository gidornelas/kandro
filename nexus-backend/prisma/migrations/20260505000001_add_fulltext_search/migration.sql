...
CREATE INDEX IF NOT EXISTS idx_kanban_cards_description_trgm ON kanban_cards USING gin (description gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_file_nodes_name_trgm ON file_nodes USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_card_thread_messages_text_trgm ON card_thread_messages USING gin (text gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_dm_messages_text_trgm ON dm_messages USING gin (text gin_trgm_ops);
